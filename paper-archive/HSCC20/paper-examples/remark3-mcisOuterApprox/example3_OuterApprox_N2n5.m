...
% Displays values for Section 5 - Remark 3 - Table 4 - n = 5.
...
% linear system. This code was adapted to our example.
...
for i = 1:length(degs)
    ...
    % performance in formulating the SDP to be solved. Especially for deg = 8.
    % Therefore, we measure the time of solving only the optimization problem
    % later.
    v1 = replace(v,x,f);
    ...
    [q4,cq4] = polynomial([x;u],deg-2);
    [q5,cq5] = polynomial([x;u],deg-2);
    ...
    [p4,cp4] = polynomial([x;u],deg-2);
    [p5,cp5] = polynomial([x;u],deg-2);
    
    [s0,cs0] = polynomial(x,deg);
    [s1,cs1] = polynomial([x;u],deg-2);
    [s2,cs2] = polynomial([x;u],deg-2);
    [s3,cs3] = polynomial([x;u],deg-2);
    [s4,cs4] = polynomial([x;u],deg-2);
    [s5,cs5] = polynomial([x;u],deg-2);
    
    % Constraints
    con = [];
    con = [con; sos( v - alpha*v1 - q0 - gx1*q1 - gx2*q2 - gx3*q3 - gx4*q4 - gx5*q5 - gu*r ); sos(q0); sos(q1); sos(q2); sos(q3); sos(q4); sos(q5); sos(r)];
    con = [con; sos( w - v - 1 - p0 - gx1*p1 - gx2*p2 - gx3*p3 - gx4*p4 - gx5*p5 ); sos(p0); sos(p1); sos(p2); sos(p3); sos(p4); sos(p5) ];
    con = [con; sos( w - s0 - gx1*s1 - gx2*s2 - gx3*s3 - gx4*s4 - gx5*s5 ); sos(s0); sos(s1); sos(s2); sos(s3); sos(s4); sos(s5) ];
    
    % Solve and time *only* for the optimization problem:
    tic
    [~,~,~,res] = solvesos(con,obj,options,[cw;cv;cq0;cq1;cq2;cq3;cq4;cq5;cr;cp0;cp1;cp2;cp3;cp4;cp5;cs0;cs1;cs2;cs3;cs4;cs5]);
    time = toc;
