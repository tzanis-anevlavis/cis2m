...
namespace cis2m {
	...
	// Compute the sigmas
	std::vector<int> ComputeSigmas(const std::vector<int>& controllability_indexes) {
		int N = controllability_indexes.size();
		std::vector<int> sigma_v(N, 0);

		if (N == 0) return sigma_v;
	...
	std::pair<MatrixXd, std::vector<int>> ComputeControllabilityIndexes(
			const MatrixXd& ControllabilityMatrix, int ControlNum) {
		...
		int indexS = 0; int rank = 0;

		std::vector<std::vector<int>> indexes(ControlNum, std::vector<int> {}); 

		int watchdog = 0;
		// Selecting the first linearly independent StateSize columns from the controllabity matrix
		...
		// Reordering the column of S to make the Cbar matrix [ref. Antsklis Linear Systems pg. 283]
	...
	MatrixXd ComputeTransformation(
			const MatrixXd& A,
			const MatrixXd& B,
			const MatrixXd& Cbar,
	...
	bool isControllable(const MatrixXd& C, int fullrank, double tol) {
		...
		if (lu_decomp.rank() != fullrank) {
			std::cerr << "[" << __FILE__ << " @ " << __LINE__ << "] System is not Controllable!" << std::endl;
		} else {
			out = true;
		}
		return out;
	...
	BrunovskyForm::BrunovskyForm(const MatrixXd& Ad, const MatrixXd& Bd) :
		...
		max_controllability_index_ = *std::max_element(
				controllability_indexes_.begin(),
				controllability_indexes_.end());
		...
		for (int i = 0; i < controllability_indexes_.size(); ++i) {
			...
			A_brunovsky_form_.block(start_index, start_index + 1, mu_i - 1, mu_i - 1) = Eigen::MatrixXd::Identity(mu_i - 1, mu_i - 1);
	...
	MatrixXd BrunovskyForm::GetDisturbanceMatrix(const MatrixXd& DisturbanceMatrix) {
