...
namespace cis2m {
	...
	std::vector<HPolyhedron> CISGenerator::ComputeShrinkedSafeSetsSequence(const HPolyhedron& ss) {
		...
		for (auto& el : SafeSet_seq) {
			std::cout << "S_k " << el.Ai().rows() << " x " << el.Ai().cols() << std::endl;
	...
	void CISGenerator::ComputeLiftedSystem(int L, int T) {
		...
		Ahd.block(0, StateDim_, StateDim_, ExtendedU2U_.cols()) = Bd_BF * ExtendedU2U_;
		...
		for (int t = 1; t < mu_max + (length - 1); t++) {
			...
			if (ThereAreInputConstraints_) {
				if (t < length) {
					...
					mcisA_ctrl.block(t * NumberInputs_, StateDim_, NumberInputs_, mcisA_cols - StateDim_) = 
						BB * Acurr.block(StateDim_, StateDim_, ExtendedInputDim_, ExtendedInputDim_);
	...
	int CISGenerator::GetStateDim() const {
		return StateDim_;
	}

	int CISGenerator::GetLevel() const {
		return Level_;
	}
}
